import { AuthorDescription } from "./reducers/authorReducer"

//User reducer types
export const SET_AUTHENTICATED = 'SET_AUTHENTICATED'
export const SET_UNAUTHENTICATED = 'SET_UNAUTHENTICATED'
export const SET_USER = 'SET_USER'
export const LOADING_USER = 'LOADING_USER'
export const STOP_LOADING_USER = 'STOP_LOADING_USER'
export const MARK_NOTIFICATION = 'MARK_NOTIFICATION'

// UI reducer types
export const SET_ERRORS = 'SET_ERRORS';
export const LOADING_UI = 'LOADING_UI';
export const CLEAR_ERRORS = 'CLEAR_ERRORS';
export const LOADING_DATA = 'LOADING_DATA';
export const STOP_LOADING_UI = 'STOP_LOADING_UI';
export const SET_ALERT = 'SET_ALERT';
export const CLOSE_ALERT = 'CLOSE_ALERT';
// Author reducer types
export const SET_AUTHORS = 'SET_AUTHORS'
export const SET_AUTHOR = 'SET_AUTHOR'
export const UPLOAD_PORTRET = 'SET_AUTHOR'
export const SET_AUTHOR_BIO = 'SET_AUTHOR_BIO'
export const ADD_AUDIO = 'ADD_AUDIO'
export const DEL_AUDIO = 'DEL_AUDIO'
export const ADD_MOVIE = 'ADD_MOVIE'
export const DEL_MOVIE = 'DEL_MOVIE'
export const ADD_ILLUSTRATION = 'ADD_ILLUSTRATION'
export const DEL_ILLUSTRATION = 'DEL_ILLUSTRATION'
export const ADD_TEST = 'ADD_TEST'
export const DEL_TEST = 'DEL_TEST'
export const ADD_BOOK = 'ADD_BOOK'
export const DEL_BOOK = 'DEL_BOOK'

//DATA
export const SET_AUDIO = 'SET_AUDIO'
export const SET_ILLUSTRATION = 'SET_ILLUSTRATION'

//TEST 
export const SET_TEST = 'SET_TEST'
export const SET_TEST_RESULT = 'SET_TEST_RESULT'
export const NEXT_QUEST = 'NEXT_QUEST'
export const ADD_ANSWER = 'ADD_ANSWER'
export const RESET_TEST = 'RESET_TEST'


//Typescript types for action
export type ActionSetAlert = {
    type: typeof SET_ALERT
    title: string
    message: string
    severity: 'success' | 'warning' | 'error'
}

export type ActionSetTest = {
    type: typeof SET_TEST
    test: Object
}

export type ActionSetTestResult = {
    type: typeof SET_TEST_RESULT
    result: any
}
export type ActionAddAnswer = {
    type: typeof ADD_ANSWER
    answer: string
}

export type ActionSetAuthors = {
    type: typeof SET_AUTHORS
    authors: AuthorDescription[]
}
